...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Encrypt or decrypt a prompt using a key"
    )
    parser.add_argument(
        "action",
    ...
    parser.add_argument(
        "--prompt_file", help="File containing the prompt to encrypt/decrypt"
    )
    parser.add_argument("--output_file", help="File to write the result to")
    parser.add_argument(
        "--key", help="Encryption key (or will use KEY env var if not provided)"
    )
    ...
    if args.action == "generate":
        ...
        if not key:
            print(
                "Error: No key provided. Use --key or set PROMPT_KEY environment variable."
            )
            exit(1)
